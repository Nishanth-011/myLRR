...
S_ini = Z_ini;
% [Z,S,U,F,E] = LRR_AGRfu(X,F_ini,Z_ini,c,lambda1,lambda2,lambda3,max_iter,Ctg);
[Z,S,U,F,E] = LRR_AGR(X,F_ini,Z_ini,c,lambda1,lambda2,lambda3,max_iter,Ctg);
% [Z,S,obj] = LRRfu(X,Z_ini,S_ini,F_ini,c,lambda1,lambda2,lambda3,max_iter);
addpath('Ncut_9');
Z_out = Z;
A = Z_out;
A = A - diag(diag(A));
