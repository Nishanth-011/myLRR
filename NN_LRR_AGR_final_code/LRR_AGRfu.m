function [Z,S,U,F,E] = LRR_AGRfu(X,F_ini,Z_ini,c,lambda1,lambda2,lambda3,max_iter,Ctg)
...
for iter = 1:max_iter
    ...
    E_old = E;
    % -------- Update Z --------- %
    Z = Ctg*(X'*(X-E+C1/miu+Y-C4/miu)+S+U-(C2+C3)/miu);
    Z = Z- diag(diag(Z));
    % Y
    P = eye(m,m);
    Q = eye(m,m);
    ...
    dist  = (distX+distX')/2+lambda1*distF;
    S     = Z+(C2-dist)/miu;
    S     = S - diag(diag(S));
    for ic = 1:n
        idx    = 1:n;
...
%     end
    % --------- obj ---------- %
    leq1 = [max(abs(L1(:))),max(abs(L2(:))),max(abs(L3(:))),max(abs(L4(:)))];
    leq1 = max(leq1);
    stopC = leq1;
    if stopC < tol
        iter
